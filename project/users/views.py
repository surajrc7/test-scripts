...
from .serializers import (TextSerializer, CitationSerializer, TimelineSerializer, OpenCitationSerializer, CoreAPISerializer)
...
class CORECitationsRefrence(GenericAPIView):  # generics.CreateAPIView
    # permission_classes = (Check_API_KEY_Auth,)
    serializer_class = TextSerializer
    ...
    def post(self, request):
        ...
        if Database:
            try:
                ...
                logging.info("Len-> Found"+str(len(rows)))
                records = main.mapsets(list(rows),remove_cols=["fullText","abstract",])
                dois = [idx['doi'] for idx in records if idx['doi']]
                result = UsersConfig.Citations.RelationsCiationRefrence(dois)
                response_dict = {"RawText": text, "ProcessedData": result,
                        'ProcessedDate': datetime.utcnow().__str__(),
                        "RequestID": request_id, "status": 200,
                        "Version": settings.VERSION}
...
class COREDocumentByCoreId(GenericAPIView):  # generics.CreateAPIView
    # permission_classes = (Check_API_KEY_Auth,)
    serializer_class = CoreAPISerializer
    
    login_schema = openapi.Schema(
        type=openapi.TYPE_OBJECT,
        properties={
            'CoreId': openapi.Schema(type=openapi.TYPE_STRING, description='CoreId to search Document'),
        },
        required=['CoreId']
    )
    ...
    def post(self, request):
        ...
        if text == '':
            return Response("Got Empty String", status=status.HTTP_400_BAD_REQUEST)
        try:
            rows = list(settings.CASSANDRA.query_coreId(collection="datasource_core",keywords=text))[0]
            if rows:
                records = main.mapdicts(rows,remove_cols=[])
                response_dict = {"RawText": text, "ProcessedData": records,
                        'ProcessedDate': datetime.utcnow().__str__(),
                        "RequestID": str(uuid.uuid4), "status": 200,
                        "Version": settings.VERSION}
                return JsonResponse(response_dict, status=200)
            else:
                response_dict = {"Error": {"Reason": "Data Unavailable",}}
                return JsonResponse(response_dict,status=401)
        except Exception as e:
            response_dict = {"Error": {"Reason": "Internal Server Error-{}".format(e)}}
            return JsonResponse(response_dict,status=503)